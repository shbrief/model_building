...
for (i in seq_len(ncol(RAVmodel))) {
  ...
  geneList <- RAVindex(RAVmodel)[,i]
  geneList <- sort(geneList, decreasing = TRUE)
  res <- GSEA(geneList, TERM2GENE = term2gene,
...
#
# # MSigDB C2 CP
# term2gene <- clusterProfiler::read.gmt("~/data2/[archive]Genomic_Super_Signature/GSEA/data/c2.cp.v7.1.symbols.gmt")
...
# dat_dir <- "~/data2/GenomicSuperSignatureLibrary/refinebioRseq"
dat_dir <- "~/data2/model_building/RAVmodel_8CRC_10OV"
RAVmodel <- readRDS(file.path(dat_dir, "RAVmodel_536/refinebioRseq_RAVmodel.rds"))
...
for (i in seq_len(ncol(RAVmodel))) {
  fname <- paste0("gsea_", i, ".rds")
  fpath <- file.path(dat_dir, "gsea_h", fname)
